...
import { updateSubCategory , getsubCategoryById , createsubServices , getallServices , subCatByCategory } from '../../redux/thunk/serviceSlice'
...
function AddCategory() {
  ...
  const getCategoryData = async () => {
    ...
    const productData = await dispatch(getsubCategoryById(data)).unwrap()
    console.log("productData",productData);
    SetSubCatDetail(productData?.datefilter[0])
    
  }


  console.log("SubCatDetail====>",subCat);
  ...
  const onFinish =  async (values) => {
   ...
   const {title } = values
    const userData = {
      title : !SubCatDetail ? title : SubCatDetail?.title ,
      catId : subCat
    }
    ...
    const newCategory = await dispatch(updateSubCategory(data)).unwrap()
          ...
          <Col lg={6} md={24} xs={24}>
            ...
            >
              ...
              >
                <Input
                  size="large"
                  placeholder={SubCatDetail?.title}
                  style={{
                    borderRadius: "5px",
                    background: "white",
                    fontSize: "14px",
              ...
              <p className="mainLabel">Select Category:</p>
                ...
                value={SubCatDetail?.catId?.title }
                onChange={(e) => {
