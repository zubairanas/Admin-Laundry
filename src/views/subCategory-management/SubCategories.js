...
import { dateAndStatusFilterSubServices , searchSubCategory , getallsubServices , getallServices , subCatByCategory , softDeleteSubServices } from '../../redux/thunk/serviceSlice'
function Categories() {
...
console.log("33335533333",cat);
  ...
  const getCategories = async (
   ...
   // );
    ...
    const response = await dispatch(getallsubServices(token)).unwrap()
      setLoading(false);
      console.log("response======>", response);
      if (response) {
        setCategories(response?.allSubCat);
        // setPaginationConfig({
  ...
  const deleteCategory = async () => {
    ...
    const resp = await dispatch(softDeleteSubServices(data)).unwrap()
    console.log("=========>" , resp);
  ...
  const onTextSearch = async () => {
    ...
    const searched = await dispatch(searchSubCategory(Searchdata)).unwrap()
...
console.log("filter?.name",filter?.name);
  ...
  const columns = [
    ...
    },
    {
        ...
        dataIndex: "catId",
        key: "catId",
        render: (value, item, index) => value ? value.title : null
      },
    {
   ...
   
    {
      ...
      <div 
        className="view-link" 
        onClick={() => navigate("/subcategories/editSubcategories/" + item._id)}
        >
  ...
  "filter.id",filter.id
    ...
    const dateFilters = await dispatch(dateAndStatusFilterSubServices(datas)).unwrap()

    const {status , data } = dateFilters
    ...
    <Layout className="configuration">
      <div className="boxDetails">
        <Row style={{ padding: "10px 20px" }}>
          ...
          >
            <h1 className="pageName">SUB CATEGORY MANAGEMENT</h1>
          </Col>
          <Col
            xs={24}
            md={12}
          ...
          >
            <Button
              ...
              onClick={() => navigate("/subcategories/addSubCategory")}
            >
              ADD SUB CATEGORY
            </Button>
          </Col>
