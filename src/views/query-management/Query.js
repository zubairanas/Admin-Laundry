...
import { QueryStatus , getAllQuery , QuerySearch ,QueryDateFilter } from '../../redux/thunk/querySlice'
function Categories() {
...
console.log("33335533333",cat);
  ...
  const getCategories = async (
   ...
   // );
    ...
    const response = await dispatch(getAllQuery(data)).unwrap()
      setLoading(false);
      console.log("response======>", response);
      if (response) {
        setCategories(response?.allqueries);
        // setPaginationConfig({
  ...
  const onTextSearch = async () => {
    ...
    const searched = await dispatch(QuerySearch(Searchdata)).unwrap()
    const {status , data } = searched
    if(status){
      Setsearchicon(data?.allqueries)
      setCategories(data?.allqueries)
    }
    else{
      getCategories();
    }
...
console.log("filter?.name",filter?.name);
  ...
  const handleDateFilter = async () => {
    ...
    const dateFilter = await dispatch(QueryDateFilter(datas)).unwrap()
  ...
  const handleChangedStatus = async (item) => {
    const userData = {
      id : item._id,
    }
    ...
    const queryStatus = await dispatch(QueryStatus(data)).unwrap()
  ...
  const columns = [
    ...
    },
    {
      ...
      </div> */}
      {
        ...
        (
          <>
            <div className="delete-link" style={{marginLeft: "10px"}} onClick={() => handleChangedStatus(item)}>
              <FaEdit style={{ fontSize: "16px", color: "black", cursor: "pointer" }}/>
            </div>
          </>
        )
      }
    ...
    <Layout className="configuration">
      <div className="boxDetails">
        <Row style={{ padding: "10px 20px" }}>
          ...
          >
            <h1 className="pageName">QUERY MANAGEMENT</h1>
          </Col>
          <Col
            xs={24}
            md={12}
